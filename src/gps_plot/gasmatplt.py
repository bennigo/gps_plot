...
def plotTime(sta, start=None, end=None, save=None, ylim=None, special=None,figDir="",events={}, fix=False,Dir=None,frfile=False,dfile=False):
    ...
    for axs, color, gastype, warning, ylim, offset in zip(maxes, colors, gastypes, warnings, ylims, offsets):
        ...
        aq_grouped.apply(plot_aq,axs,"yellow")
        ...
        #print( shift.describe() )
        aq_grouped.apply(plot_aq,axs,"lightgrey")


        # Only plot data with low humitity
        aq_grouped = data.loc[rh_mask].groupby('aquisition')[gastype]
        aq_grouped.apply(plot_aq,axs,color)
    ...
    for aq in aquisitions:
        ...
        taxs.plot(x,rh,linewidth=2,color='red')

    taxs.axhline(y=93, linewidth=2, color = "grey")
    ## ---------------------------------------------
    if events:
...
def addTimeofPlot(fig,dstr="%Y%m%d"):
    ...
    maxes = fig.axes
    for axs in maxes:
        axs.axvline(x=timeofPlot, color='grey', linestyle="--",  zorder=2) 
